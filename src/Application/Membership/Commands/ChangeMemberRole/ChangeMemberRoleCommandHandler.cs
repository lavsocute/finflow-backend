...
public sealed class ChangeMemberRoleCommandHandler : ICommandHandler<ChangeMemberRoleCommand, Result>
{
    ...
    public async Task<Result> Handle(ChangeMemberRoleCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!_authorizationService.CanChangeMemberRole(request.ActorMembershipId, request.MembershipId, actor.Role))
            return Result.Failure(TenantMembershipErrors.NotFound);
