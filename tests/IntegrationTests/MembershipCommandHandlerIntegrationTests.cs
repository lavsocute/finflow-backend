...
public sealed class MembershipCommandHandlerIntegrationTests
{
    ...
    public async Task SwitchWorkspaceCommandHandler_RotatesRefreshToken_AndReturnsTargetMembership()
    {
        ...
        var account = scope.SeedAccount("handler.switch@finflow.test", "P@ssw0rd!");
    ...
    [Fact]
    public async Task SwitchWorkspaceCommandHandler_AllowsAccountScopedSession_ToBridgeIntoOwnedMembership()
    {
        using var scope = _fixture.CreateScope();

        var tenant = scope.SeedTenant("Bridge Workspace", "handler-bridge-account-session");
        var account = scope.SeedAccount("handler.bridge@finflow.test", "P@ssw0rd!");
        var membership = scope.SeedMembership(account.Id, tenant.Id, RoleType.Manager);
        const string rawRefreshToken = "handler-bridge-account-refresh-token";
        ...
        var result = await handler.Handle(
            new SwitchWorkspaceCommand(new SwitchWorkspaceRequest(account.Id, membership.Id, rawRefreshToken)),
            CancellationToken.None);
        ...
        Assert.True(refreshTokens[1].IsActive);
        Assert.Equal(membership.Id, refreshTokens[1].MembershipId);
    }
    ...
    public async Task InviteMemberCommandHandler_CreatesInvitation_AndReturnsRawToken()
    {
        ...
        var inviterAccount = scope.SeedAccount("handler.inviter@finflow.test", "P@ssw0rd!");
    ...
    public async Task AcceptInviteCommandHandler_ForExistingAccount_AddsMembership_AndIssuesTokens()
    {
        ...
        var existingAccount = scope.SeedAccount("handler.accept@finflow.test", "P@ssw0rd!");
        scope.SeedMembership(existingAccount.Id, sourceTenant.Id, RoleType.Staff);

        var inviterAccount = scope.SeedAccount("handler.admin@finflow.test", "P@ssw0rd!");
